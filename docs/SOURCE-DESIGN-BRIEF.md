...
- Visual grammar: black automotive backdrop, acid-yellow accent, rounded pill CTAs, dark service cards, yellow vehicle hero/service imagery, compact testimonial cards, three pricing cards, tabbed FAQ/accordion runtime, and blog thumbnails.
...
- Dependency modernization: AOS/jQuery reveal behavior is rebuilt as static Astro, CSS transitions, and visible-by-default reveal-safe content. Bootstrap source interaction patterns are preserved where they define the section shape, including the FAQ vertical pill tabs and collapse accordions.
...
- FAQ: source FAQ must stay as a Bootstrap-style runtime section: top heading/contact row, three left-side vertical pill buttons (`General Questions`, `Services & Process Questions`, `Pricing & Payment Questions`), right-side tab panes, and five accordion dropdowns per pane. It must not be flattened into a single static FAQ list or custom-only one-column accordion.
